//! F46 — Azure Blob credentials in the OS keychain. Keyed on
//! source_id. Mirrors sftp_creds / webdav_creds / dropbox_creds.

use crate::azure_blob::AzureBlobCredentials;
use crate::error::{AgentError, Result};
use once_cell::sync::Lazy;
use std::collections::HashMap;
use std::sync::Mutex;

const SERVICE: &str = "sery-link-azure";

static CRED_CACHE: Lazy<Mutex<HashMap<String, AzureBlobCredentials>>> =
    Lazy::new(|| Mutex::new(HashMap::new()));

pub fn save(source_id: &str, creds: &AzureBlobCredentials) -> Result<()> {
    if !creds.is_valid() {
        return Err(AgentError::Config(
            "Azure credentials need a SAS token".to_string(),
        ));
    }
...
pub fn load(source_id: &str) -> Result<Option<AzureBlobCredentials>> {
