//! F48 — Dropbox credentials in the OS keychain. Keyed on
//! source_id. Mirrors sftp_creds / webdav_creds shape.

use crate::dropbox::DropboxCredentials;
use crate::error::{AgentError, Result};
use once_cell::sync::Lazy;
use std::collections::HashMap;
use std::sync::Mutex;

const SERVICE: &str = "sery-link-dropbox";

static CRED_CACHE: Lazy<Mutex<HashMap<String, DropboxCredentials>>> =
    Lazy::new(|| Mutex::new(HashMap::new()));

pub fn save(source_id: &str, creds: &DropboxCredentials) -> Result<()> {
    if !creds.is_valid() {
...
pub fn load(source_id: &str) -> Result<Option<DropboxCredentials>> {
