...
/// Ask the configured BYOK LLM a single question, run as a text-to-
/// SQL agent loop against the user's local files. The two-step
/// flow (generate SQL → execute → interpret) lives in `text_to_sql`;
/// this command just resolves the provider/key/model and forwards.
///
/// Privacy: schemas + sample rows + the SQL + the result table go
/// to the configured BYOK provider. That's the same destination as
/// before, just more bytes. Audit log records the call.
#[tauri::command]
pub async fn ask_byok(prompt: String) -> Result<crate::text_to_sql::AgentAnswer, String> {
    ...
    crate::text_to_sql::ask(provider, &api_key, &prompt, model.as_deref())
        .await
        .map_err(|e| e.to_string())
}
